...
---
uid: Global.SlimeCreature
canonical_path: /api/Global/Creature/SlimeCreature
---
...
## Derived
[SlimeGirl](/api/Global/Misc/SlimeGirl), [SlimeLover](/api/Global/Misc/SlimeLover), [SlimePawn](/api/Global/Misc/SlimePawn)
...
### MotionDelayTime
```csharp
protected static float MotionDelayTime { get; }
```
...
### SoundTerm
```csharp
protected static float SoundTerm { get; }
```
...
```csharp
public virtual SoundEffectPlayer SoundMake(string src, float vol = 1)
```
...
```csharp
public virtual SoundEffectPlayer SoundMake(string src, Vector3 pos, float vol = 1)
```
...
### SoundMakeLoop(string, float)
```csharp
public virtual SoundEffectPlayer SoundMakeLoop(string src, float vol = 1)
```
