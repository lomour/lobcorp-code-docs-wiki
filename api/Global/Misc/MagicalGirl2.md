...
---
uid: Global.MagicalGirl_2
canonical_path: /api/Global/Misc/MagicalGirl2
---
...
```csharp
public class MagicalGirl_2 : CreatureBase
```

## Inheritance
[object](https://learn.microsoft.com/dotnet/api/system.object) → [CreatureBase](/api/Global/Creature/CreatureBase) → MagicalGirl_2

## Constructors

### MagicalGirl_2()
```csharp
public MagicalGirl_2()
```
...
### _animScript
```csharp
private MagicalGirl_2Anim _animScript
```

#### Field Value
**Type:** Global.MagicalGirl_2Anim

### attackDelays
```csharp
private List<MagicalGirl_2.AttackDelay> attackDelays
```

#### Field Value
**Type:** System.Collections.Generic.List{MagicalGirl_2.AttackDelay}

### attackSpace
```csharp
private const float attackSpace = 0.75
```
...
### beforeChargeTimer
```csharp
private Timer beforeChargeTimer
```
...
### beforeEscapeDelay
```csharp
private const float beforeEscapeDelay = 3
```
...
### bloodCycleFreq
```csharp
private const float bloodCycleFreq = 0.7
```
...
### bloodMakeTime
```csharp
private const float bloodMakeTime = 30
```
...
### deads
```csharp
private List<WorkerModel> deads
```
...
### laughFreq
```csharp
private const float laughFreq = 20
```
...
### portalSpace
```csharp
private const float portalSpace = 1.8
```
...
### removed
```csharp
private List<WorkerModel> removed
```
...
### sound_Laugh
```csharp
private const string sound_Laugh = "Laugh"
```
...
### sound_Move
```csharp
private const string sound_Move = "Move"
```
...
### sound_Warp
```csharp
private const string sound_Warp = "Warp"
```
...
### swallowSpace
```csharp
private const float swallowSpace = 0.1
```
...
### transformDelay
```csharp
private const float transformDelay = 1.5
```
...
### animScript
```csharp
public MagicalGirl_2Anim animScript { get; }
```
...
### MakeChewEffect(UnitDirection)
```csharp
private void MakeChewEffect(UnitDirection dir)
```
...
### SetAttackNode()
```csharp
private void SetAttackNode()
```

### Swallow(UnitModel)
```csharp
private void Swallow(UnitModel unit)
```
