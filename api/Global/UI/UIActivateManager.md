...
Holds the camera used for UI (a reference to the camera on the [ManualUI](/api/Global/UI/Manual/ManualUI)) and responsible for closing some UIs when the background is clicked.

Also, closes out of the UIs when it is notified of a 'TurnOffUI' event. 
...
Holds an [IActivatableObject](/api/Global/UI/NarrationLoggerUI/NarrationLoggerUI), though...)
...
A copy of the world camera from [ManualUI](/api/Global/UI/Manual/ManualUI).
