...
UI that appears above (or below) [agent units](/api/Global/UI/Agent-UI/AgentContinueUI), department, tool status, HP, and SP, once Yesod's research for it is obtained.

See [AgentUnitUI](/api/Global/UI/Agent-UI/AgentUnitUI) for before Yesod's upgrade.
